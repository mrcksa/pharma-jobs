...
// ─── NOTA: La API de Anthropic requiere una API key.
// En producción, NUNCA expongas la key en el frontend.
// Usa un proxy o backend (ej. Vercel Functions, Netlify Functions).
// Para pruebas locales puedes usar: import.meta.env.VITE_ANTHROPIC_API_KEY
// y crear un archivo .env con: VITE_ANTHROPIC_API_KEY=sk-ant-...
// ─────────────────────────────────────────────────────────────────────
...
const css = `
  .app{font-family:var(--font-sans);max-width:740px;margin:0 auto;padding:0 0 2rem}
  ...
  .tabs{display:flex;gap:4px;margin-bottom:1.25rem;background:var(--color-background-secondary);border-radius:10px;padding:4px}
  .tab{flex:1;border:none;background:none;border-radius:8px;padding:8px 4px;cursor:pointer;font-size:13px;font-weight:400;color:var(--color-text-secondary);transition:all .15s;white-space:nowrap}
  .tab.active{background:var(--color-background-primary);color:var(--color-text-primary);font-weight:500;box-shadow:0 1px 4px rgba(0,0,0,.08)}
  ...
  .filters select{flex:1;min-width:110px;font-size:13px}
  .results-label{font-size:13px;color:var(--color-text-secondary);margin-bottom:12px}
  .job-grid{display:grid;gap:10px}
  .job-card{background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:14px;padding:1rem 1.25rem;cursor:pointer;transition:all .15s}
  ...
  .job-title{margin:0;font-weight:500;font-size:15px;color:var(--color-text-primary)}
  .job-company{margin:2px 0 0;font-size:13px;color:var(--color-text-secondary)}
  ...
  .tag-neutral{background:var(--color-background-secondary);color:var(--color-text-secondary)}
  .job-footer{display:flex;justify-content:space-between;align-items:center;margin-top:10px;padding-top:10px;border-top:0.5px solid var(--color-border-tertiary)}
  .salary{font-size:13px;font-weight:500;color:var(--color-text-primary)}
  ...
  .match-low{background:var(--color-background-secondary);color:var(--color-text-secondary)}
  .section-label{font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;color:var(--color-text-secondary);margin:0 0 8px}
  .profile-card{background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:14px;padding:1.25rem;margin-bottom:16px}
  ...
  .upload-area{border:1.5px dashed var(--color-border-secondary);border-radius:14px;padding:2rem;text-align:center;cursor:pointer;transition:all .15s;margin-bottom:14px}
  .upload-area:hover{border-color:#378ADD;background:#E6F1FB22}
  .divider{display:flex;align-items:center;gap:10px;margin:16px 0;color:var(--color-text-secondary);font-size:13px}
  .divider::before,.divider::after{content:'';flex:1;height:0.5px;background:var(--color-border-tertiary)}
  .primary-btn{width:100%;padding:10px;background:linear-gradient(135deg,#185FA5,#1D9E75);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:500;cursor:pointer;transition:opacity .15s}
  .primary-btn:hover{opacity:.88}
  .primary-btn:disabled{opacity:.45;cursor:not-allowed}
  .secondary-btn{padding:8px 16px;background:var(--color-background-secondary);color:var(--color-text-primary);border:0.5px solid var(--color-border-secondary);border-radius:8px;font-size:13px;cursor:pointer}
  .platform-card{background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:14px;padding:1rem 1.25rem;display:flex;align-items:center;gap:14px;transition:all .15s}
  .platform-card:hover{border-color:var(--color-border-secondary)}
  ...
  .modal{background:var(--color-background-primary);border-radius:16px;padding:1.5rem;max-width:540px;width:100%;max-height:82vh;overflow-y:auto}
  .modal-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px}
  .close-btn{background:var(--color-background-secondary);border:none;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:16px;color:var(--color-text-secondary);display:flex;align-items:center;justify-content:center;flex-shrink:0}
  ...
  .alert-card{background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:14px;padding:1.25rem;margin-bottom:12px}
  .alert-card h3{margin:0 0 12px;font-size:15px;font-weight:500}
  .form-row{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px}
  .form-row input,.form-row select{flex:1;min-width:140px;font-size:13px}
  .form-label{font-size:13px;color:var(--color-text-secondary);margin:0 0 6px;display:block}
  .alert-result{background:var(--color-background-secondary);border-radius:10px;padding:14px;font-size:13px;line-height:1.8;color:var(--color-text-primary);white-space:pre-wrap;margin:12px 0}
    ...
    <div className={`job-card${isFav?" fav":""}`} onClick={() => onClick(job, score)}>
      <div className="job-card-top">
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          ...
          <button className="fav-btn" onClick={e => { e.stopPropagation(); onToggleFav(job.id); }}>
            {isFav ? "❤️" : "🤍"}
          </button>
        </div>
      </div>
      ...
      <div className="job-footer">
        ...
        <span style={{ fontSize:12, color:"var(--color-text-secondary)" }}>Ver detalles →</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="modal-overlay" onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="modal">
        <div className="modal-header">
          <div style={{ display:"flex", gap:12, alignItems:"center" }}>
            ...
            <div>
              ...
              <p style={{ margin:0, fontSize:13, color:"var(--color-text-secondary)" }}>{job.company} · Remoto</p>
            </div>
          </div>
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
        ...
        <p style={{ fontSize:14, color:"var(--color-text-secondary)", marginBottom:16, lineHeight:1.7 }}>{job.desc}</p>
...
export default function App() {
  ...
  // Persistencia con localStorage (en producción fuera de Claude)
  useEffect(() => {
    try {
      const f = localStorage.getItem("pharma_favs");
      if (f) setFavs(JSON.parse(f));
      const p = localStorage.getItem("pharma_profile");
      if (p) setProfile(JSON.parse(p));
      const e = localStorage.getItem("pharma_alertEmail");
      if (e) setAlertEmail(e);
    } catch {}
  }, []);
  ...
  const toggleFav = (id) => {
    ...
    setFavs(next);
    try { localStorage.setItem("pharma_favs", JSON.stringify(next)); } catch {}
  };
  ...
  async function processProfile(content, isFile, base64) {
    if (!API_KEY) { setUploadMsg("Añade tu API key en el archivo .env (VITE_ANTHROPIC_API_KEY)"); setUploading(false); return; }
    ...
    try {
      ...
      setProfile(parsed);
      try { localStorage.setItem("pharma_profile", JSON.stringify(parsed)); } catch {}
      setUploadMsg(`Perfil listo: ${parsed.name||"Cargado"}`);
      setTab("search");
    } catch { setUploadMsg("Error al procesar. Comprueba tu API key."); }
    setUploading(false);
  }
  ...
  function handleUpload(e) {
    ...
    reader.onload=ev=>processProfile(null,true,ev.target.result.split(",")[1]);
  ...
  return (
    <>
      ...
      <div className="app">
            ...
            {alertResult && (
              <div className="alert-card">
                ...
                <div className="copy-row">
                  ...
                  {alertEmail && (
                    <button className="primary-btn" style={{ flex:1 }} onClick={mailtoAlert}>Enviar a {alertEmail} →</button>
                  )}
                </div>
              </div>
            )}
          ...
          <div>
            {profile ? (
              <div>
                <div className="profile-card">
                  <div style={{ display:"flex",gap:14,alignItems:"center",marginBottom:12 }}>
                    ...
                    <div>
                      ...
                      <p style={{ margin:0,fontSize:13,color:"var(--color-text-secondary)" }}>{profile.email||""}</p>
                    </div>
                  </div>
                  <p style={{ fontSize:14,color:"var(--color-text-secondary)",margin:"0 0 10px",lineHeight:1.6 }}>{profile.summary}</p>
                ...
                <button className="secondary-btn" onClick={()=>{ setProfile(null); try{localStorage.removeItem("pharma_profile");}catch{} }}>Eliminar perfil</button>
              </div>
            ) : (
              <div>
                <div className="upload-area" onClick={()=>fileRef.current?.click()}>
                  ...
                  <p style={{ margin:"4px 0 0",fontSize:13,color:"var(--color-text-secondary)" }}>La IA extrae tus habilidades y experiencia automáticamente</p>
                </div>
                ...
                {!API_KEY && <p style={{ fontSize:12,color:"#D85A30",marginTop:10 }}>Crea un archivo .env con VITE_ANTHROPIC_API_KEY=sk-ant-... para activar el análisis con IA.</p>}
              </div>
            )}
        ...
        {tab==="platforms" && (
          <div>
            <p style={{ fontSize:14,color:"var(--color-text-secondary)",marginBottom:20 }}>Accede a otras plataformas con búsquedas preconfiguradas para el sector.</p>
            {profile && (
              ...
              {PLATFORMS.map(p=>(
                <div key={p.name} className="platform-card">
                  ...
                  <div>
                    ...
                    <p style={{ margin:0,fontSize:12,color:"var(--color-text-secondary)" }}>Remoto · farmacéutico / hospitalario</p>
                  </div>
                  <a href={p.url} target="_blank" rel="noreferrer" className="plat-link">Abrir →</a>
                </div>
              ))}
            </div>
            <p style={{ fontSize:12,color:"var(--color-text-secondary)",marginTop:20 }}>El registro en cada plataforma debe realizarse manualmente.</p>
          </div>
        )}
